    ...
    public static class SemillaPermisos
    {
        public static void Sembrar(ModelBuilder builder)
        {
            ...
            builder.Entity<RolPermiso>().HasData(
                Enumerable.Range(1, 15).Select(i => new RolPermiso { Id = i, Rol = 1, PermisoId = i }).ToArray()
            );

            // Administrador - todo menos Usuarios.Gestionar
            builder.Entity<RolPermiso>().HasData(
                Enumerable.Range(1, 14).Select(i => new RolPermiso { Id = 15 + i, Rol = 2, PermisoId = i }).ToArray()
            );

            // Vendedor - ventas, productos ver, clientes, caja
            builder.Entity<RolPermiso>().HasData(
                new RolPermiso { Id = 30, Rol = 3, PermisoId = 1  },
                new RolPermiso { Id = 31, Rol = 3, PermisoId    = 2  },
                new RolPermiso { Id = 32, Rol = 3, PermisoId = 6  },
                new RolPermiso { Id = 33, Rol = 3, PermisoId = 9  },
                new RolPermiso { Id = 34, Rol = 3, PermisoId = 10 },
                new RolPermiso { Id = 35, Rol = 3, PermisoId = 12 },
                new RolPermiso { Id = 36, Rol = 3, PermisoId = 13 }
            );
        }
    }
}
