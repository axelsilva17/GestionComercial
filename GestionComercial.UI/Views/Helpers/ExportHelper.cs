    ...
    public static class ExportHelper
    {
        ...
        /// </summary>
         public static void ExportarAuditoriaCaja(List<CajaAuditoriaItemDto> cajas)
         {
             ...
             // Headers (agregado Turno como 3ra columna)
             string[] headers = { "Fecha", "Hora", "Turno", "Usuario Apertura", "Monto Inicial", "Ventas Efectivo", "Efectivo en Caja", "Monto Final", "Diff s/Efectivo", "Diff c/Efectivo", "Estado" };
             for (int i = 0; i < headers.Length; i++)
             {
             ...
             for (int i = 0; i < cajas.Count; i++)
             {
                 ...
                 ws.Cell(row, 3).Value = c.Turno;
                 ws.Cell(row, 4).Value = c.UsuarioApertura;
                 ws.Cell(row, 5).Value = (double)c.MontoInicial;
                 ws.Cell(row, 5).Style.NumberFormat.Format = "$ #,##0.00";
                 ws.Cell(row, 6).Value = (double)c.VentasEfectivo;
                 ws.Cell(row, 6).Style.NumberFormat.Format = "$ #,##0.00";
                 ws.Cell(row, 7).Value = (double)c.EfectivoEnCaja;
                 ws.Cell(row, 7).Style.NumberFormat.Format = "$ #,##0.00";
                 ws.Cell(row, 8).Value = c.MontoFinal.HasValue ? (double)c.MontoFinal.Value : 0;
                 ws.Cell(row, 8).Style.NumberFormat.Format = "$ #,##0.00";
                 ws.Cell(row, 9).Value = (double)c.DiferenciaSinEfectivo;
                 ws.Cell(row, 9).Style.NumberFormat.Format = "$ #,##0.00";
                 ws.Cell(row, 10).Value = (double)c.DiferenciaConEfectivo;
                 ws.Cell(row, 10).Style.NumberFormat.Format = "$ #,##0.00";
                 ws.Cell(row, 11).Value = c.Estado;

                 // Color diferencia
                 if (c.DiferenciaConEfectivo != 0)
                 {
                     ws.Cell(row, 10).Style.Font.FontColor = XLColor.FromHtml("#EF4444");
                 }
             }
